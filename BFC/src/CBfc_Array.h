...
template<class baseClass> BFC_TEMPLATE_CLASS 
	Array : public CBfc_Object
{
public:
	Array(unsigned int uListChunk=16, unsigned int uDataChunk=1)
	...
	Array(const Array<baseClass> &copy)
		: m_uCount(0), m_uListLen(0), m_bcpList(NULL)
	...
	void Remove (const baseClass &cmp) {
		INT64 f = Find(cmp);
		if (f >= 0) RemoveAt (f);
	}
	...
	bool ArrayEquals(const Array<baseClass> &other) {
...
template<class baseClass> inline
	ULONG Array<baseClass>::StartIteration() const {
	return 0;
}

template<class baseClass> inline
	baseClass	*Array<baseClass>::GetNext(ULONG &uIterationIndex) const {
	if (uIterationIndex < GetCount()) return m_bcpList[uIterationIndex++];
	return NULL;
}


// typedef BFC_API Array<int> CBfc_IntArray;       // not applicable yet!

typedef		Array<STRING>					StringArray;
typedef		Array<			bool>				ArrayBool;
typedef		Array<			char>				ArrayChar;
typedef		Array<	unsigned	char>		ArrayUnsignedChar;
typedef		Array<			short>				ArrayShort;
typedef		Array<	unsigned	short>		ArrayUnsignedShort;
typedef		Array<			int>					ArrayInt;
typedef		Array<	unsigned	int>			ArrayUnsignedInt;
typedef		Array<			long>				ArrayLong;
typedef		Array<	unsigned	long>		ArrayUnsignedLong;
typedef		Array<			INT64>			ArrayInt64;
typedef		Array<			UINT64>			ArrayUnsignedInt64;
typedef		Array<			float>				ArrayFloat;
typedef		Array<			double>			ArrayDouble;
typedef		Array<		long	double>		ArrayLongDouble;

// Later: Make a special class for this one to avoid inlines.
typedef		BFC_API Array<BYTEPTR>			ArrayPtr;

} // namespace CBfc

#define BFC_ITERATE(ar,ptr,idx) idx = ar.StartIteration(); while ((ptr=ar.GetNext(idx)))

#endif   // ndef _BCARRAY_H
...
#if 0
/*
template<class baseClass> BFC_TEMPLATE_CLASS Array : public CBfc_Object
{
public:
	Array(unsigned int uListChunk=16, unsigned int uDataChunk=1);
	~Array();
...
private:
	baseClass	    *m_bcpList;
	UINT64	    	m_uListLen;
	unsigned int	m_uListChunk, m_uDataChunk;
	__uint64        m_uCount;
