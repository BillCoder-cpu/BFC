...
********************************************************************************/
//
// CBfc coding should look like htis: (please see my numerous examples)
//
//  class AppWindow : public Window
//  {
//    public:							// Constructor - takes a bfc resource ID.
//        AppWindow (Resource_ID resourceID);
//	  protectd:
//        virtual int OnMouseMove ();	// This is an override so we get control of the main window.
//    // etc.
//  };
//
//   CBfcGUI      GUI;                        // There could be several running at once.
//   AppWindow    Mainwindow(RESOURCE_ID);    // This should be derived from Window
//
//   bOk = GUI.LaunchGUI (MainWindow);        // Starts up our Gui - messages become active and we are alerted when things happen.
//
...
FXDEFMAP(CFoxGLWindow) CFoxGLWindowMap[]=
{
  //________Message_Type_____________________ID____________Message_Handler_______
  FXMAPFUNC(SEL_PAINT,              0, CFoxBFCWindow::onFxPaint),
//  FXMAPFUNC(SEL_PAINT,             CFoxBFCWindow::ID_CANVAS, CFoxBFCWindow::onFxPaint),
...
FXIMPLEMENT(CFoxGLWindow,FXGLViewer,CFoxGLWindowMap,ARRAYNUMBER(CFoxGLWindowMap))
CFoxGLWindow::CFoxGLWindow(GUI *p_GUI,AppLayerPtr p_App,const Resource_ID resourceID, UILayerWindow* p_Layer, UILayerWindow *p_Parent)
	: FXGLViewer(p_Parent->GetFoxWindow()->GetAsFoxContainer(),p_GUI->GetVisual(),p_Parent->GetFoxWindow()->GetAsFox(),resourceID)
	, CFoxBFCWindow(WND_GL, (FXApp*)p_App, p_Layer)
{
}

FXWindow *CFoxGLWindow::GetAsFox() const
{
	return (FXWindow *)this;
}

long CFoxGLWindow::HandleAsBase(FXObject*p_sender,FXSelector fxsel,void* ptr)
{
	return FXGLViewer::handle(p_sender,fxsel,ptr);
}
...
//	CFoxBFCWindow_derivedFrom::resize(w,h);
	Size2dU	size(w,h);
	Message	msg(BFCGUI_MSG_RESIZE,&size);
...
long CFoxBFCWindow::onFxKeyRelease(FXObject*p_sender,FXSelector fxsel,void* ptr)
{
	...
	Message msg(BFCGUI_MSG_KEYRELEASE,ev->code);
	if (OnMessage(msg) == BFCGUIDEFS_MESSAGE_PROCESSED)
		return BFCGUIDEFS_MESSAGE_PROCESSED;
...
//	  m_p_GLViewer->grab();
	...
	Message msg(BFCGUI_MSG_MOUSEDOWN,(ev->win_y << 16 | ev->win_x));
	if (OnMessage(msg) == BFCGUIDEFS_MESSAGE_PROCESSED)
		return BFCGUIDEFS_MESSAGE_PROCESSED;
	return HandleMessage();
}
...
*/
	const Message msg(BFCGUI_MSG_MOUSEMOVE,(ev->win_y << 16 | ev->win_x));
	if (OnMessage(msg) == BFCGUIDEFS_MESSAGE_PROCESSED)
		return BFCGUIDEFS_MESSAGE_PROCESSED;
	return HandleMessage();
}
...
*/
	const Message msg(BFCGUI_MSG_MOUSEUP,(ev->win_y << 16 | ev->win_x));
	if (OnMessage(msg) == BFCGUIDEFS_MESSAGE_PROCESSED)
		return BFCGUIDEFS_MESSAGE_PROCESSED;
	return HandleMessage();
}
...
long CFoxBFCWindow::onFxMouseEnter(FXObject*p_sender,FXSelector fxsel,void* ptr)
{
	RecordMessage(p_sender, fxsel, ptr);
	FXEvent *ev=(FXEvent*) ptr;
	const Message msg(BFCGUI_MSG_MOUSEENTER,(ev->win_y << 16 | ev->win_x));
	if (OnMessage(msg) == BFCGUIDEFS_MESSAGE_PROCESSED)
		return BFCGUIDEFS_MESSAGE_PROCESSED;
	return HandleMessage();
}
...
long CFoxBFCWindow::onFxMouseLeave(FXObject*p_sender,FXSelector fxsel,void* ptr)
{
	RecordMessage(p_sender, fxsel, ptr);
	FXEvent *ev=(FXEvent*) ptr;
	const Message msg(BFCGUI_MSG_MOUSELEAVE,(ev->win_y << 16 | ev->win_x));
	if (OnMessage(msg) == BFCGUIDEFS_MESSAGE_PROCESSED)
		return BFCGUIDEFS_MESSAGE_PROCESSED;
	return HandleMessage();
}
